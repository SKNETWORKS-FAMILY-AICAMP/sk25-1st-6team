...
def render():
    ...
    - DB 직접 접근 금지
    - (현재 단계) 내부 더미(SUBSIDY_MAP, get_dummy_stations) + MockApiClient 흐름 일부 사용
    """
    # ✅ 팀 규칙 흐름 맞추기: 지역 목록은 MockApiClient에서 가져오는 형태로 한 번이라도 사용
    regions = MockApiClient.get_regions()  # List[RegionDTO]
    region_names = [r.name for r in regions]  # 예: "서울특별시", "경기도" ...
    ...
    with st.container():
        with st.form("subsidy_form", clear_on_submit=False):
            ...
            # ✅ 기존 DO_OPTIONS / SI_OPTIONS 대신, MockApiClient 기반 이름을 써도 되지만
            # 지금은 화면 유지 위해 기존 옵션을 유지합니다.
            r2 = st.columns(2)
            with r2[0]:
    ...
    # ✅ 팀원 요구 대응: MockApiClient에서 가져온 region_names를 화면에 1줄로라도 사용
